...
class SessionsGroupByCategoryDataLoaders extends DataLoadersFactory
{
    /**
     * @param array $args
     *
    ...
    public function get($args)
    {
        ...
        return $this->mergeDatasByKey('category', $current, $previous);
    }
    ...
    private function getDatas($visitsKpi, $dateRange)
    {
        ...
        $sessions = [];
        if (!empty($values['byCategory'])) {
            foreach ($values['byCategory'] as $source) {
                $session = [
                    'category' => $source['category'],
                    'currentValue' => $source['sessions_by_account'],
                ];
